...
class FilesList(Resource):
    ...
    def post(self):
        ...
        fileUploadData = request.form.to_dict()
        objs = []
        for f in files:
            if "ARCHIVE" in f.filename:
                objs.extend(uploads.multiple_upload(file=f, **fileUploadData))
            elif "FLAGS" in f.filename:
                try:
                    uploads.flags_upload(file=f, **fileUploadData)
                except:
                    return {"success": False, "errors": "Flags parsing error"}, 400
            else:
                obj = uploads.upload_file(file=f, **fileUploadData)
                objs.append(obj)
