...
function getSolves(id) {
  return CTFd.api.get_challenge_solves({ challengeId: id }).then(response => {
    ...
    var word = " Решений";
    const solves = parseInt(data.length)
    if (solves === 1) {
      word = " Решение";
    } else if (solves < 5) {
      word = " Решения";
    ...
    $(".challenge-solves").text(solves + word);
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      box.append(
        '<tr class="bordered"><td><a class="user-text-f" href="{0}">{2}</td><td>{3}</td></tr><tr><td class="p-0"></td><td class="p-1"></td></tr>'.format(
          account_url,
          id,
          htmlEntities(name),
          date
...
function loadChals() {
  return CTFd.api.get_challenge_list().then(function(response) {
      ...
      if ($.inArray(challenges[i].category, categories) == -1) {
        ...
        const categoryrow = $(
          "" +
            ...
            '<div class="category-header col-md-12 mb-5 label-f">' +
            "</div>" +
    ...
    for (let i = 0; i <= challenges.length - 1; i++) {
      ...
      const chalwrap = $(
        "<div id='{0}' class='col-md-6 col-lg-4 d-inline-block'></div>".format(chalid)
      );
      let chalbutton;

      if (solves.indexOf(chalinfo.id) == -1) {
        chalbutton = $(
          "<button class='btn btn-dark challenge-button w-100 text-truncate pt-3 pb-3 mb-2 label-f text-center' value='{0}'></button>".format(
            chalinfo.id
          )
        );
      } else {
        chalbutton = $(
          "<button class='btn btn-dark challenge-button solved-challenge w-100 text-truncate pt-3 pb-3 mb-2 label-f text-center' value='{0}'><i class='fas fa-check corner-button-check'></i></button>".format(
            chalinfo.id
          )
        );
      }
