...
from CTFd.models import LogSubmission, AnticheatEvents, Fails, Flags, Hints, HintUnlocks, Solves, Submissions, Tags, db
...
from CTFd.utils.telegram.firstblood import firstblood

from CTFd.utils.telegram.bot import send_message
from CTFd.utils.logging import log
    ...
    def get(self, query_args):
        ...
        for challenge in chal_q:
            ...
            if is_admin() and request.args.get("view") == "admin":
                res["file_hash"] = challenge.file_hash
            response.append(
                res
            )
...
class Challenge(Resource):
    ...
    def get(self, challenge_id):
        ...
        if authed():
            ...
            max_id = max([x.participant for x in chal.files if x.participant != None] + [1])
            ...
            for f in chal.files:
                if f.participant == None or (f.participant % max_id) == _id or is_admin():
                    token = {
                        "user_id": user.id,
        ...
        if get_config("user_mode") == 'users':
            is_opend = LogSubmission.query\
                    .filter_by(team=user.name)\
                    .filter_by(challenge=chal.name)\
                    .first()
        else:
            is_opend = LogSubmission.query\
                    .filter_by(team=team.name)\
                    .filter_by(challenge=chal.name)\
                    .first()

        if not is_opend:
            if get_config("user_mode") == 'users':
                log_open = LogSubmission(
                    team=user.name,
                    challenge=chal.name,
                    ip_open=get_ip(),
                    user_open=user.name,
    ...
    def post(self):
        ...
        if current_user.is_admin():
            ...
            if preview:
                ...
                chal_class = get_chal_class(challenge.type)
                user = get_current_user()
                status, message = chal_class.attempt(user, challenge, request)
            ...
            return (
                {
                    ...
                    "data": {
                        "status": "paused",
                        "message": "{} приостановлено".format(config.ctf_name()),
                    },
                },
                403,
            )
            ...
            return (
                {
                    ...
                    "data": {
                        "status": "ratelimited",
                        "message": "Вы отправляете флаг слишком быстро. Помедленнее.",
                    },
                },
                429,
            )
        ...
        if not solves:
            ...
            if status:  # The challenge plugin says the input is right
                if ctftime() or current_user.is_admin():
                    ...
                    clear_standings()
                    Model = get_model()
                    all_solves = Solves.query.join(Model, Solves.account_id == Model.id).filter(
                    ...
                    if not is_admin():
                        user = get_current_user()
                        if tg_notifs() and all_solves == 1:
                            log(
                                "submissions",
                                "[{date}] {name} was the first to solve {challenge_id} [FIRSTBLOOD]",
                                challenge_id=challenge_id,
                            )
                            firstblood(user=user, challenge=challenge)
                ...
                if get_config("user_mode") == 'users':
                    log_submit = LogSubmission.query\
                    .filter_by(team=user.name)\
                    .filter_by(challenge=challenge.name)\
                    .first()
                else:
                    log_submit = LogSubmission.query\
                    .filter_by(team=team.name)\
                ...
                log_submit.ip_submit = get_ip()
                log_submit.user_submit = user.name
                log_submit.date_submit = datetime.datetime.now()
                time_delta = datetime.datetime.now() - log_submit.date_open
                log_submit.time_delta = time_delta.seconds
                db.session.add(log_submit)
                db.session.commit()
                ...
                if not is_admin():
                    if time_delta.seconds < 60:
                        ...
                        if get_config("user_mode") == 'teams':
                            with_team = f"из команды [{team.name}]({request.host}/admin/teams/{team.id})"

                        send_message(
                            message=f"RedBoard alerts: ВОЗМОЖНЫЙ ШАРИНГ ФЛАГОВ - СЛИШКОМ БЫСТРО\n[{user.name}]({request.host}/admin/users/{user.id}) {with_team} решил задание *{challenge.name}* менее чем за 1 минуту",
                            mtype='markdown'
                        )

                        with_team = ""
                        if get_config("user_mode") == 'teams':
                            with_team = f"from {team.name}"

                        content = f"{user.name} {with_team} solved the task {challenge.name} less than 1 minute [TOO FAST]"
