...
class UserSchema(ma.ModelSchema):
    ...
    email = field_for(
        ...
        allow_none=False,
        validate=[
            validate.Email("Недопустимый формат почты"),
            validate.Length(min=1, max=128, error="Почта не может быть пустой"),
        ],
    )
    website = field_for(
        Users,
        ...
        if is_admin():
            ...
            if user_id:
                if existing_user and existing_user.id != user_id:
                    raise ValidationError(
                        "Имя пользователя уже существует", field_names=["name"]
                    )
            else:
                if existing_user:
                    if current_user:
            ...
            if name == current_user.name:
                ...
                if bool(name_changes) is False:
                    raise ValidationError(
                        "Изменение имени запрещено", field_names=["name"]
                    )
                if existing_user:
                    raise ValidationError(
                        "Имя пользователя уже существует", field_names=["name"]
    ...
    def validate_email(self, data):
        ...
        if is_admin():
            ...
            if user_id:
                if existing_user and existing_user.id != user_id:
                    raise ValidationError(
                        "Данная почта уже существует", field_names=["email"]
                    )
            else:
                if existing_user:
                    if current_user:
        ...
        if is_admin():
            ...
            for f in fields:
                ...
                if field.required is True and value.strip() == "":
                    raise ValidationError(
                        f"Поле '{field.name}' обязательно", field_names=["fields"]
                    )

                if field.editable is False and entry is not None:
                    raise ValidationError(
                        f"Поле '{field.name}' не может быть изменено",
                        field_names=["fields"],
                    )
