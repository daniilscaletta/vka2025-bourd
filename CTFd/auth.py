...
from CTFd.utils.config.integrations import telegram_registration
...
from CTFd.utils.telegram import verify
...
def reset_password(data=None):
    if config.can_send_mail() is False:
        return render_template(
            ...
            errors=[
                markup(
                    "Отправка почтового сообщения не настроена.<br> Пожалуйста, свяжитесь с организаторами соревнования для сброса пароля."
                )
            ],
        )
    ...
    if data is not None:
        ...
        if request.method == "POST":
            ...
            if pass_short:
                return render_template(
                    "reset_password.html", errors=["Пожалуйста, выберите длинный пароль"]
                )
    ...
    if request.method == "POST":
        ...
        if not user:
            return render_template(
                "reset_password.html",
                infos=[
                    "Если аккаунт уже существует, то вы получили сообщение на почту."
                ],
            )
...
def register():
    ...
    if request.method == "POST":
        ...
        if not valid_email:
            errors.append("Пожалуйста, введите почту в правильном формате")
        ...
        if names:
            errors.append("Имя пользователя уже используется")
        if team_name_email_check is True:
            errors.append("Почта и имя пользователя не должны совпадать")
        if emails:
            errors.append("Данная почта уже используется")
        if pass_short:
...
@auth.route("/tgoauth")
def tgoauth_login():
    token = get_app_config("TG_BOT_SECRET") or get_config('tg_bot_secret')
    if verify(request, token) and -60 <= time.time() - int(request.args['auth_date']) <= 60:
        tg_id = request.args['id']
        username = request.args.get('username', '')
        if not username:
            log("logins", "[{date}] {ip} - Registration via telegram without username blocked")
            error_for(
                endpoint="auth.login",
                message="Registration via telegram without username blocked. Please add username to your telegram account.",
            )
            return redirect(url_for("auth.login"))

        user = Users.query.filter_by(oauth_id=tg_id).first()
        if user is None:
            # Check if we are allowing registration before creating users
            if registration_visible() or telegram_registration():
                user = Users(
                    name=username,
                    oauth_id=tg_id,
                    verified=True,
                ...
                log("logins", "[{date}] {ip} - Public registration via telegram blocked")
                error_for(
                    endpoint="auth.login",
